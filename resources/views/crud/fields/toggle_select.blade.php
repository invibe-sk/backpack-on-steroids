    ...
    <select name="{{$field['name']}}" id="{{$field['name']}}" class="form-control">
        @if( isset($field['options']) && is_array($field['options']) )
            @foreach ($field['options'] as $value => $label )
                @php ($optionPointer++)
                <option value="{{ $value }}" {{$optionValue !== '' && $optionValue == $value ? ' selected': ''}}>{{ $label }}</option>
            @endforeach
        @endif
    </select>
    ...
    @push('crud_fields_scripts')
        <script>
            ...
            //     };
                ...
                let hideWhenFields = {!! json_encode($field['hide_when']) !!};

                $('select[name="{{$field['name']}}"]').on('change', function(){
                    // return $toggle( $(this) );
                    let selected = $(this).val();

                    handleShowHide(selected);
                });

                handleShowHide($('select[name="{{$field['name']}}"]').val());

                function handleShowHide(value) {
                    let hideFields = hideWhenFields[value] ? hideWhenFields[value] : null;

                    // hide fields
                    if (hideFields) {
                        hideFields.forEach((name) => {
                            $("input[name='" + name + "'], select[name='" + name + "']").closest('.form-group').hide();
                        })
                    }

                    Object.keys(hideWhenFields).filter((element) => {
                        return element !== value;
                    }).forEach((key) => {
                        let showFields = hideWhenFields[key] ? hideWhenFields[key] : null;

                        if (showFields) {
                            showFields.forEach((name) => {
                                if(!hideFields || !hideFields.includes(name)) {
                                    $("input[name='" + name + "'], select[name='" + name + "']").closest('.form-group').show();
                                }
                            })
                        }
